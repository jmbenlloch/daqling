...
bool Receiver::sleep_receive(daqling::utilities::Binary &bin) {
  if (receive(bin)) {
    return true;
  }
...
// Queue receiver
QueueReceiver::QueueReceiver(uint chid, std::unique_ptr<Queue> ptr) : Receiver(chid) {
  m_queue = std::move(ptr);
}

bool QueueReceiver::receive(daqling::utilities::Binary &bin) {
...
bool QueueReceiver::sleep_receive(daqling::utilities::Binary &bin) {
  if (m_queue->sizeGuess() != 0) {
...
bool QueueReceiver::addReceiveHandler() {
  ...
  m_handler_thread = std::thread([&]() {
    while (!m_stop_handler) {
      daqling::utilities::Binary msg;
      if (m_chained_receiver->sleep_receive(msg)) {
        // NOLINTNEXTLINE(misc-use-after-move)
        while (!m_queue->sleep_write(std::move(msg)) && !m_stop_handler) {
