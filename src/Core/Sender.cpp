...
bool Sender::sleep_send(const daqling::utilities::Binary &bin) {
  if (send(bin)) {
    return true;
  }
...
// Queue sender
QueueSender::QueueSender(uint chid, std::unique_ptr<Queue> ptr) : Sender(chid) {
  m_queue = std::move(ptr);
}
bool QueueSender::send(const daqling::utilities::Binary &bin) { return m_queue->write(bin); }
bool QueueSender::sleep_send(const daqling::utilities::Binary &bin) {
  ...
  m_handler_thread = std::thread([&]() {
    while (!m_stop_handler) {
      ...
      if (m_queue->sizeGuess() != 0) {
        if (m_queue->sleep_read(msg)) {
          m_chained_sender->send(msg);
        }
      } else {
